...
export async function GET(
  ...
  try {
    ...
    const survey = await SurveyModel.findOne({ _id: params.surveyId, owner: userId });

    if (!survey) {
        return NextResponse.json({ success: false, message: 'Survey not found or you are not the owner' }, { status: 404 });
    }

    const responses = await ResponseModel.find({ surveyId: params.surveyId });
